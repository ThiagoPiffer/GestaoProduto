...
namespace GestaoProduto.Dominio.Produtos
{
    public class ArmazenadorProduto
    {
        ...
        {
            ValidadorDeRegra.Novo()
                .Quando(_produtoRepositorio.Consultar().Where(p => p.Ativo && p.Descricao == produtoDto.Descricao && p.Id != produtoDto.Id).Count() > 0, ChaveTextos.ProdutoJaCadastrado)
                .Quando(_fornecedorRepositorio.Consultar().Where(f => f.Ativo && f.CNPJ == produtoDto.Fornecedor.CNPJ).Count() == 0, ChaveTextos.CNPJInexistente)
                .Quando(produtoDto.DataFabricacao >= produtoDto.DataValidade, ChaveTextos.DataFabricacaoMaiorQueDataValidade)
                .DispararExcecaoSeExistir();
